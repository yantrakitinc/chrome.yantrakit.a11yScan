...
export function renderCrawlProgressHtml(
  ...
  return `
    <div class="progress-bar" role="status" aria-live="polite" aria-atomic="true">
      <div style="display:flex;align-items:center;margin-bottom:4px;gap:var(--ds-space-4)">
...
export function renderPageRuleWaitHtml(info: { url: string; description: string; waitType?: string } | null): string {
  return `
    <div role="alert" aria-live="assertive" class="fs-0" style="padding:var(--ds-space-4) var(--ds-space-6);border-bottom:2px solid var(--ds-yellow-400);background:var(--ds-amber-50)">
      ...
      <div style="display:flex;gap:var(--ds-space-3);flex-wrap:wrap">
        <button id="continue-crawl" class="cur-pointer min-h-24" style="padding:var(--ds-space-2) var(--ds-space-5);font-size:11px;font-weight:700;color:var(--ds-amber-cta-fg);background:var(--ds-amber-500);border:none;border-radius:var(--ds-radius-3)">Continue</button>
        <button id="scan-then-continue" class="cur-pointer min-h-24" style="padding:var(--ds-space-2) var(--ds-space-5);font-size:11px;font-weight:700;color:var(--ds-zinc-700);background:#fff;border:1px solid var(--ds-zinc-300);border-radius:var(--ds-radius-3)">Scan page, then continue</button>
        <button id="cancel-wait" class="cur-pointer min-h-24" style="font-size:11px;font-weight:700;color:var(--ds-red-600);background:none;border:1px solid var(--ds-red-200);border-radius:var(--ds-radius-3);margin-left:auto;padding:var(--ds-space-2) var(--ds-space-5)">Cancel</button>
      </div>
    </div>
  `;
}
