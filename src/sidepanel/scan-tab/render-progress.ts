  ...
  return `
    <div class="progress-bar" role="status" aria-live="polite" aria-atomic="true">
      <div style="display:flex;align-items:center;margin-bottom:4px;gap:8px">
        <span class="fs-0 font-mono" style="font-size:var(--ds-text-base);font-weight:700;color:var(--ds-zinc-600)">${pageLabel}</span>
...
export function renderPageRuleWaitHtml(info: { url: string; description: string; waitType?: string } | null): string {
  return `
    <div role="alert" aria-live="assertive" class="fs-0" style="padding:8px 12px;border-bottom:2px solid var(--ds-yellow-400);background:var(--ds-amber-50)">
      <div style="font-size:var(--ds-text-base);font-weight:700;color:var(--ds-amber-900);margin-bottom:6px">⚠ Page rule triggered</div>
      ${info?.description ? `<div style="font-size:var(--ds-text-base);color:var(--ds-zinc-800);margin-bottom:4px">${escHtml(info.description)}</div>` : ""}
      ${info?.url ? `<div class="truncate font-mono" style="font-size:var(--ds-text-sm);color:var(--ds-zinc-500);margin-bottom:6px" title="${escHtml(info.url)}">${escHtml(info.url)}</div>` : ""}
      <div style="display:flex;gap:6px;flex-wrap:wrap">
        <button id="continue-crawl" class="cur-pointer min-h-24" style="padding:4px 10px;font-size:var(--ds-text-base);font-weight:700;color:var(--ds-amber-cta-fg);background:var(--ds-amber-500);border:none;border-radius:4px">Continue</button>
        <button id="scan-then-continue" class="cur-pointer min-h-24" style="padding:4px 10px;font-size:var(--ds-text-base);font-weight:700;color:var(--ds-zinc-700);background:#fff;border:1px solid var(--ds-zinc-300);border-radius:4px">Scan page, then continue</button>
        <button id="cancel-wait" class="cur-pointer min-h-24" style="font-size:var(--ds-text-base);font-weight:700;color:var(--ds-red-600);background:none;border:1px solid var(--ds-red-200);border-radius:4px;margin-left:auto;padding:4px 10px">Cancel</button>
      </div>
    </div>
  `;
}
