...
export function renderAriaResultsHtml(widgets: iAriaWidget[]): string {
  ...
  return `
    <div class="scan-pane">
      ...
      ${issues.length > 0 ? `<div style="font-size:var(--ds-text-base);font-weight:800;color:var(--ds-zinc-500);text-transform:uppercase;letter-spacing:0.05em;margin-bottom:var(--ds-space-2)">Issues</div>` : ""}
      ${issues.map((w) => renderAriaWidget(w, false)).join("")}
      ${compliant.length > 0 ? `<div style="font-size:var(--ds-text-base);font-weight:800;color:var(--ds-zinc-500);text-transform:uppercase;letter-spacing:0.05em;margin:var(--ds-space-4) 0 var(--ds-space-2)">Compliant</div>` : ""}
...
export function renderAriaWidget(w: iAriaWidget, pass: boolean): string {
  return `
    <details${pass ? "" : " open"} style="border:1px solid ${pass ? "var(--ds-green-200)" : "var(--ds-red-200)"};border-radius:4px;background:${pass ? "var(--ds-green-50)" : "var(--ds-red-50)"};margin-bottom:4px">
      <summary class="cur-pointer" style="list-style:none;display:flex;align-items:center;gap:var(--ds-space-4);padding:var(--ds-space-4);font-size:var(--ds-text-base)">
      ...
      <div class="scan-detail-body">
        ${w.checks.filter((c) => !c.pass).map((c) => `
          <div style="font-size:var(--ds-text-base);color:var(--ds-red-700);padding:var(--ds-space-1) 0 var(--ds-space-1) var(--ds-space-4);border-left:2px solid var(--ds-red-200)">${escHtml(c.message)}</div>
        `).join("")}
        ${w.checks.filter((c) => c.pass).map((c) => `
