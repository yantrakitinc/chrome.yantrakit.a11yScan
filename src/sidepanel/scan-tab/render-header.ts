...
export function renderExpandedToggleHtml(s: {
  ...
  return `
    <select id="wcag-version" aria-label="WCAG version" ${s.busy ? "disabled" : ""} style="font-size:var(--ds-text-md);padding:4px 6px;border:1px solid var(--ds-zinc-300);border-radius:4px;font-weight:600">
    ...
    <select id="wcag-level" aria-label="Conformance level" ${s.busy ? "disabled" : ""} style="font-size:var(--ds-text-md);padding:4px 6px;border:1px solid var(--ds-zinc-300);border-radius:4px;font-weight:600">
    ...
    <div style="display:flex;align-items:center;gap:2px">
      ...
      ${s.hasTestConfig ? '<span style="font-size:var(--ds-text-sm);font-weight:700;color:var(--ds-amber-600);background:var(--ds-amber-100);border:1px solid var(--ds-amber-300);border-radius:4px;padding:1px 5px;white-space:nowrap">Config loaded</span>' : ""}
    </div>
    <button id="reset-btn" aria-label="Reset all settings" ${s.busy ? "disabled" : ""} class="cur-pointer min-h-24" style="font-size:var(--ds-text-base);font-weight:700;color:var(--ds-red-600);background:none;border:1px solid var(--ds-red-200);border-radius:4px;padding:4px 10px">Reset</button>
...
export function renderCollapsedToggleHtml(s: {
  ...
  if (modes.length === 0) {
    ...
    modeHtml = modes.map((m) => `<span style="font-size:var(--ds-text-base);font-weight:600;padding:2px 6px;border-radius:4px;${modeColors[m as string] || "background:var(--ds-zinc-200);color:var(--ds-zinc-700)"}">${m}</span>`).join(" ");
  } else {
    modeHtml = `<span style="font-size:var(--ds-text-base);font-weight:600;padding:2px 6px;border-radius:4px;background:var(--ds-zinc-200);color:var(--ds-zinc-700)">${modes.length} modes</span>`;
  }
          ...
          <div style="display:flex;flex-wrap:wrap;gap:4px;align-items:center;margin-bottom:4px">
            ${s.viewports.map((v, i) => `
              <input type="number" min="320" value="${v}" data-index="${i}" class="vp-input font-mono min-h-24" aria-label="Viewport ${i + 1} width in pixels"
                style="width:60px;font-size:var(--ds-text-base);font-weight:600;padding:2px 4px;border:1px solid var(--ds-zinc-300);border-radius:4px;background:#fff;color:var(--ds-zinc-800);box-sizing:border-box">
              <button type="button" class="vp-remove cur-pointer min-h-24" data-index="${i}" aria-label="Remove ${v}px viewport"
                style="font-size:var(--ds-text-md);font-weight:700;line-height:1;padding:2px 5px;border:1px solid var(--ds-zinc-300);border-radius:4px;background:#fff;color:var(--ds-zinc-600)"
                ${s.viewports.length <= 1 ? "disabled" : ""}>×</button>
            `).join("")}
          </div>
          <div style="display:flex;gap:6px;align-items:center">
            <button type="button" id="vp-add"
              class="cur-pointer min-h-24" style="font-size:var(--ds-text-base);font-weight:700;padding:2px 8px;border:1px solid var(--ds-zinc-300);border-radius:4px;background:#fff;color:var(--ds-zinc-800)"
              ${s.viewports.length >= 6 ? "disabled" : ""}>+ add</button>
            <button type="button" id="vp-done"
              class="cur-pointer min-h-24" style="font-size:var(--ds-text-base);font-weight:700;padding:2px 8px;border:1px solid var(--ds-amber-600);border-radius:4px;background:var(--ds-amber-100);color:var(--ds-amber-800)">done</button>
          </div>
        </div>`
      : `<div style="display:flex;align-items:center;gap:4px;padding-left:24px;flex-wrap:wrap">
          ${s.viewports.map((v) => `<span class="font-mono" style="font-size:var(--ds-text-base);font-weight:600;color:var(--ds-zinc-700);background:#fff;border:1px solid var(--ds-zinc-300);border-radius:4px;padding:2px 6px">${v}</span>`).join("")}
          <button type="button" id="vp-edit"
            class="cur-pointer min-h-24" style="font-size:var(--ds-text-base);font-weight:700;padding:1px 6px;border:none;background:none;color:var(--ds-indigo-700);text-decoration:underline">edit</button>
        </div>`
    : "";
