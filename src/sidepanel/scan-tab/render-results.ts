...
export function renderResults(
  ...
  const mvBanner = mvResult ? `
    <div style="padding:6px 10px;background:var(--ds-amber-100);border:1px solid var(--ds-amber-300);border-radius:6px;margin-bottom:6px;font-size:var(--ds-text-base);font-weight:600;color:var(--ds-amber-800)">
  ...
  return `
    <div class="scan-pane">
      ...
      <details style="margin-top:8px">
        <summary class="cur-pointer" style="list-style:none;font-size:var(--ds-text-md);font-weight:700;color:var(--ds-green-700);padding:6px 0;display:flex;align-items:center;gap:6px">
        ...
        <div>
          ${result.passes.map((p) => `
            <details style="border-bottom:1px solid var(--ds-zinc-100)">
              <summary class="cur-pointer" style="list-style:none;display:flex;align-items:center;gap:8px;padding:4px 8px;font-size:var(--ds-text-base)">
              ...
              <div style="padding:2px 8px 6px 28px">
                <div style="font-size:var(--ds-text-base);color:var(--ds-zinc-600);margin-bottom:4px">${escHtml(p.description)}</div>
                ${p.nodes.map((n) => `
                  <div class="font-mono" style="font-size:var(--ds-text-base);color:var(--ds-green-700);padding:2px 8px;margin:1px 0;background:var(--ds-green-50);border-radius:3px;display:flex;align-items:center;gap:6px;overflow:hidden">
...
export function renderViolation(v: iScanResult["violations"][0], viewportWidths: number[] | null = null): string {
  ...
  const vpBadge = viewportWidths && viewportWidths.length > 0
    ? viewportWidths.map((w) => `<span class="font-mono" style="font-size:var(--ds-text-sm);font-weight:700;padding:1px 4px;background:var(--ds-blue-100);color:var(--ds-sky-700);border-radius:3px;margin-left:2px">${w}px</span>`).join("")
    : "";
  return `
    <details class="severity-${v.impact} sr-details" style="border-radius:0 4px 4px 0;margin-bottom:4px">
      <summary class="scan-detail-summary">
        ...
        <span class="fs-0" style="font-weight:700;padding:2px 6px;border-radius:4px;font-size:var(--ds-text-base)">${v.impact}</span>
        <span class="fs-0 font-mono" style="color:var(--ds-zinc-600);font-weight:700">${v.nodes.length}</span>
      </summary>
      <div class="scan-detail-body">
        ${v.wcagCriteria && v.wcagCriteria.length > 0 ? `<div style="margin-bottom:6px">${v.wcagCriteria.map((c) => `<a href="${getWcagUrl(c)}" target="_blank" rel="noopener" style="font-size:var(--ds-text-base);font-weight:700;color:var(--ds-indigo-700);text-decoration:underline;margin-right:8px">${c} — Learn more ↗</a>`).join("")}</div>` : ""}
        ${v.nodes.map((n) => `
          <div style="background:#fff;border:1px solid var(--ds-zinc-200);border-radius:4px;padding:6px;margin-bottom:4px;font-size:var(--ds-text-base)">
            ...
            <button class="explain-btn cur-pointer min-h-24" data-rule="${v.id}" data-description="${escHtml(v.description)}" style="display:none;font-size:var(--ds-text-base);font-weight:700;color:var(--ds-indigo-700);background:none;border:none;margin-top:4px">Chat about it →</button>
          </div>
        `).join("")}
      </div>
    </details>
...
export function renderCrawlResultsHtml(
  ...
  const toggle = `
    <div role="group" aria-label="Group crawl results by" style="display:flex;gap:0;border:1px solid var(--ds-zinc-300);border-radius:4px;overflow:hidden;margin-bottom:8px">
      <button type="button" id="crawl-view-page" aria-pressed="${crawlViewMode === "page"}"
        class="f-1 cur-pointer min-h-24" style="padding:4px 8px;font-size:var(--ds-text-base);font-weight:700;border:none;background:${crawlViewMode === "page" ? "var(--ds-amber-100)" : "#fff"};color:${crawlViewMode === "page" ? "var(--ds-amber-800)" : "var(--ds-zinc-600)"}">By page</button>
      <button type="button" id="crawl-view-wcag" aria-pressed="${crawlViewMode === "wcag"}"
        class="f-1 cur-pointer min-h-24" style="padding:4px 8px;font-size:var(--ds-text-base);font-weight:700;border:none;border-left:1px solid var(--ds-zinc-300);background:${crawlViewMode === "wcag" ? "var(--ds-amber-100)" : "#fff"};color:${crawlViewMode === "wcag" ? "var(--ds-amber-800)" : "var(--ds-zinc-600)"}">By WCAG</button>
    </div>
  `;
  ...
  if (crawlViewMode === "page") {
    body = allUrls.map((url) => {
      ...
      if (err) {
        return `
          <details style="border:1px solid var(--ds-red-200);border-radius:4px;margin-bottom:4px;background:var(--ds-red-50)">
            ...
            <div style="padding:4px 8px 8px;font-size:var(--ds-text-base);color:var(--ds-red-700)">${escHtml(err)}</div>
          </details>
        `;
      }
      ...
      return `
        <details style="border:1px solid ${hasViolations ? "var(--ds-red-200)" : "var(--ds-green-200)"};border-radius:4px;margin-bottom:4px;background:${hasViolations ? "var(--ds-red-50)" : "var(--ds-green-50)"}">
          ...
          <div class="scan-detail-body">
            ${r.violations.sort((a, b) => severityOrder(a.impact) - severityOrder(b.impact)).map((v) => renderViolation(v)).join("") || '<div style="font-size:var(--ds-text-base);color:var(--ds-green-700);padding:4px 0">No violations found.</div>'}
          </div>
        </details>
      `;
    }).join("");
    ...
    if (byCriterion.size === 0) {
      body = '<div style="padding:12px;font-size:var(--ds-text-md);color:var(--ds-green-700);font-weight:600;text-align:center">No violations found across all pages.</div>';
    } else {
