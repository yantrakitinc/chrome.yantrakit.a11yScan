...
export function renderResults(
  ...
  const mvBanner = mvResult ? `
    <div style="padding:var(--ds-space-3) var(--ds-space-5);background:var(--ds-amber-100);border:1px solid var(--ds-amber-300);border-radius:6px;margin-bottom:var(--ds-space-3);font-size:var(--ds-text-base);font-weight:600;color:var(--ds-amber-800)">
  ...
  return `
    <div class="scan-pane">
      ...
      <details style="margin-top:var(--ds-space-4)">
        <summary class="cur-pointer" style="list-style:none;font-size:var(--ds-text-md);font-weight:700;color:var(--ds-green-700);padding:var(--ds-space-3) 0;display:flex;align-items:center;gap:var(--ds-space-3)">
        ...
        <div>
          ${result.passes.map((p) => `
            <details style="border-bottom:1px solid var(--ds-zinc-100)">
              <summary class="cur-pointer" style="list-style:none;display:flex;align-items:center;gap:var(--ds-space-4);padding:var(--ds-space-2) var(--ds-space-4);font-size:var(--ds-text-base)">
              ...
              <div style="padding:var(--ds-space-1) var(--ds-space-4) var(--ds-space-3) 28px">
                <div style="font-size:var(--ds-text-base);color:var(--ds-zinc-600);margin-bottom:var(--ds-space-2)">${escHtml(p.description)}</div>
                ${p.nodes.map((n) => `
                  <div class="font-mono" style="font-size:var(--ds-text-base);color:var(--ds-green-700);padding:var(--ds-space-1) var(--ds-space-4);margin:1px 0;background:var(--ds-green-50);border-radius:var(--ds-radius-2);display:flex;align-items:center;gap:var(--ds-space-3);overflow:hidden">
...
export function renderViolation(v: iScanResult["violations"][0], viewportWidths: number[] | null = null): string {
  ...
  const vpBadge = viewportWidths && viewportWidths.length > 0
    ? viewportWidths.map((w) => `<span class="font-mono" style="font-size:var(--ds-text-sm);font-weight:700;padding:1px var(--ds-space-2);background:var(--ds-blue-100);color:var(--ds-sky-700);border-radius:var(--ds-radius-2);margin-left:var(--ds-space-1)">${w}px</span>`).join("")
    : "";
  return `
    <details class="severity-${v.impact} sr-details" style="border-radius:0 var(--ds-radius-3) var(--ds-radius-3) 0;margin-bottom:var(--ds-space-2)">
      <summary class="scan-detail-summary">
        <svg class="chevron" aria-hidden="true" width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4l3 3 3-3"/></svg>
        <b class="truncate f-1" style="color:var(--ds-zinc-900)">${v.wcagCriteria?.join(", ") || v.id}${vpBadge}</b>
        <span class="fs-0" style="font-weight:700;padding:var(--ds-space-1) var(--ds-space-3);border-radius:var(--ds-radius-3);font-size:var(--ds-text-base)">${v.impact}</span>
        <span class="fs-0 font-mono" style="color:var(--ds-zinc-600);font-weight:700">${v.nodes.length}</span>
      </summary>
      <div class="scan-detail-body">
        ${v.wcagCriteria && v.wcagCriteria.length > 0 ? `<div style="margin-bottom:var(--ds-space-3)">${v.wcagCriteria.map((c) => `<a href="${getWcagUrl(c)}" target="_blank" rel="noopener" style="font-size:var(--ds-text-base);font-weight:700;color:var(--ds-indigo-700);text-decoration:underline;margin-right:var(--ds-space-4)">${c} — Learn more ↗</a>`).join("")}</div>` : ""}
        ${v.nodes.map((n) => `
...
export function renderCrawlResultsHtml(
  ...
  const toggle = `
    <div role="group" aria-label="Group crawl results by" style="display:flex;gap:0;border:1px solid var(--ds-zinc-300);border-radius:var(--ds-radius-3);overflow:hidden;margin-bottom:var(--ds-space-4)">
      <button type="button" id="crawl-view-page" aria-pressed="${crawlViewMode === "page"}"
        class="f-1 cur-pointer min-h-24" style="padding:var(--ds-space-2) var(--ds-space-4);font-size:var(--ds-text-base);font-weight:700;border:none;background:${crawlViewMode === "page" ? "var(--ds-amber-100)" : "#fff"};color:${crawlViewMode === "page" ? "var(--ds-amber-800)" : "var(--ds-zinc-600)"}">By page</button>
      <button type="button" id="crawl-view-wcag" aria-pressed="${crawlViewMode === "wcag"}"
        class="f-1 cur-pointer min-h-24" style="padding:var(--ds-space-2) var(--ds-space-4);font-size:var(--ds-text-base);font-weight:700;border:none;border-left:1px solid var(--ds-zinc-300);background:${crawlViewMode === "wcag" ? "var(--ds-amber-100)" : "#fff"};color:${crawlViewMode === "wcag" ? "var(--ds-amber-800)" : "var(--ds-zinc-600)"}">By WCAG</button>
    </div>
  `;
  ...
  if (crawlViewMode === "page") {
    body = allUrls.map((url) => {
      ...
      if (err) {
        return `
          <details style="border:1px solid var(--ds-red-200);border-radius:var(--ds-radius-3);margin-bottom:var(--ds-space-2);background:var(--ds-red-50)">
            <summary class="scan-detail-summary">
              <svg class="chevron" aria-hidden="true" width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4l3 3 3-3"/></svg>
            ...
            <div style="padding:var(--ds-space-2) var(--ds-space-4) var(--ds-space-4);font-size:var(--ds-text-base);color:var(--ds-red-700)">${escHtml(err)}</div>
          </details>
        `;
      }
      ...
      return `
        <details style="border:1px solid ${hasViolations ? "var(--ds-red-200)" : "var(--ds-green-200)"};border-radius:4px;margin-bottom:4px;background:${hasViolations ? "var(--ds-red-50)" : "var(--ds-green-50)"}">
          <summary class="scan-detail-summary">
            ...
            <span class="truncate f-1 font-mono" style="color:var(--ds-zinc-800)" title="${escHtml(url)}">${escHtml(url)}</span>
            <span class="fs-0" style="font-size:var(--ds-text-sm);font-weight:700;color:${hasViolations ? "var(--ds-red-700)" : "var(--ds-green-700)"}">${hasViolations ? violationCount + " issue" + (violationCount === 1 ? "" : "s") : passCount + " pass"}</span>
          </summary>
          <div class="scan-detail-body">
            ${r.violations.sort((a, b) => severityOrder(a.impact) - severityOrder(b.impact)).map((v) => renderViolation(v)).join("") || '<div style="font-size:var(--ds-text-base);color:var(--ds-green-700);padding:var(--ds-space-2) 0">No violations found.</div>'}
          </div>
        </details>
      `;
    }).join("");
    ...
    if (byCriterion.size === 0) {
      body = '<div style="padding:var(--ds-space-6);font-size:var(--ds-text-md);color:var(--ds-green-700);font-weight:600;text-align:center">No violations found across all pages.</div>';
    } else {
      body = Array.from(byCriterion.entries()).map(([criterion, entries]) => {
        ...
        return `
          <details class="severity-${entries[0].violation.impact}" style="border-radius:0 var(--ds-radius-3) var(--ds-radius-3) 0;margin-bottom:var(--ds-space-2)">
            <summary class="scan-detail-summary">
              <svg class="chevron" aria-hidden="true" width="10" height="10" viewBox="0 0 10 10" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 4l3 3 3-3"/></svg>
              ...
              <span class="fs-0" style="font-size:var(--ds-text-sm);color:var(--ds-zinc-600)">${uniquePages.length} page${uniquePages.length === 1 ? "" : "s"}</span>
              ...
              ${uniquePages.map((pageUrl) => {
                ...
                return `
                  <div style="margin-bottom:var(--ds-space-2)">
                    <div class="truncate font-mono" style="font-size:var(--ds-text-sm);color:var(--ds-zinc-600)margin-bottom:2px" title="${escHtml(pageUrl)}">${escHtml(pageUrl)}</div>
