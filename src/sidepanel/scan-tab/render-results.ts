        ...
        ${v.nodes.map((n) => `
          <div style="background:#fff;border:1px solid var(--ds-zinc-200);border-radius:4px;padding:6px;margin-bottom:4px;font-size:11px">
            <div style="display:flex;justify-content:space-between;gap:4px">
              ...
              <button type="button" class="highlight-btn fs-0 cur-pointer min-h-24" data-selector="${escHtml(n.selector)}" aria-label="Highlight ${escHtml(n.selector)} on the page" style="font-size:11px;font-weight:700;color:var(--ds-amber-700);background:none;border:none">Highlight</button>
            </div>
            <div style="color:var(--ds-red-700);margin-top:2px">${escHtml(n.failureSummary)}</div>
            <button type="button" class="explain-btn cur-pointer min-h-24" data-rule="${v.id}" data-description="${escHtml(v.description)}" style="display:none;font-size:11px;font-weight:700;color:var(--ds-indigo-700);background:none;border:none;margin-top:4px">Chat about it →</button>
          </div>
        `).join("")}
      </div>
    </details>
