...
export function openConfigDialog(opts: iConfigDialogOptions): void {
  ...
  content.innerHTML = `
    ...
    <div style="display:flex;align-items:center;gap:var(--ds-space-3);flex-wrap:wrap">
      <button id="config-apply-btn" class="f-1 cur-pointer min-h-24" style="padding:var(--ds-space-4);font-size:var(--ds-text-md);font-weight:800;color:var(--ds-amber-cta-fg);background:var(--ds-amber-500);border:none;border-radius:var(--ds-radius-3)">Apply</button>
      ...
      ${state.testConfig ? '<button id="config-clear-btn" class="cur-pointer min-h-24" style="padding:var(--ds-space-2) var(--ds-space-5);font-size:var(--ds-text-base);font-weight:700;color:var(--ds-red-600);background:none;border:1px solid var(--ds-red-200);border-radius:var(--ds-radius-3)">Clear Config</button>' : ""}
    </div>
  `;
