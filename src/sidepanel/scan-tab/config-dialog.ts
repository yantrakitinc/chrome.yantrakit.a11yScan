...
export function openConfigDialog(opts: iConfigDialogOptions): void {
  ...
  content.innerHTML = `
    ...
    <textarea id="config-textarea" aria-label="Paste config JSON here" placeholder='Paste JSON config here, e.g. { "wcag": { "version": "2.1", "level": "AA" } }' class="font-mono" style="width:100%;box-sizing:border-box;font-size:var(--ds-text-base);padding:8px;border:1px solid ${state.testConfig ? "var(--ds-amber-300)" : "var(--ds-zinc-300)"};border-radius:4px;resize:vertical;min-height:100px;background:#fff;color:var(--ds-zinc-800);line-height:1.5">${escHtml(configJson)}</textarea>
    ...
    <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
      <button id="config-apply-btn" class="f-1 cur-pointer min-h-24" style="padding:8px;font-size:var(--ds-text-md);font-weight:800;color:var(--ds-amber-cta-fg);background:var(--ds-amber-500);border:none;border-radius:4px">Apply</button>
      <label id="config-upload-label" class="cur-pointer min-h-24" style="padding:4px 10px;font-size:var(--ds-text-base);font-weight:700;color:var(--ds-zinc-700);background:#fff;border:1px solid var(--ds-zinc-300);border-radius:4px;display:flex;align-items:center">
        Upload .json
      ...
      ${state.testConfig ? '<button id="config-clear-btn" class="cur-pointer min-h-24" style="padding:4px 10px;font-size:var(--ds-text-base);font-weight:700;color:var(--ds-red-600);background:none;border:1px solid var(--ds-red-200);border-radius:4px">Clear Config</button>' : ""}
    </div>
  `;
