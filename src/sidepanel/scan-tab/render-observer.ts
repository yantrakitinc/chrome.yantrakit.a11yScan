  ...
  return filtered.map((entry) => `
    <div role="button" tabindex="0" aria-label="Open observer entry: ${escHtml(entry.title || entry.url)}" style="padding:8px;border:1px solid var(--ds-zinc-200);border-radius:4px;background:#fff;margin-bottom:4px" class="observer-entry cur-pointer" data-url="${escHtml(entry.url)}">
      <div style="display:flex;align-items:center;gap:6px">
        ...
        ${entry.viewportBucket ? `<span class="fs-0" style="font-size:var(--ds-text-sm);color:var(--ds-sky-700);background:var(--ds-blue-100);padding:1px 4px;border-radius:3px">${escHtml(entry.viewportBucket)}</span>` : ""}
      </div>
      <div class="truncate font-mono" style="font-size:var(--ds-text-sm);color:var(--ds-zinc-500);margin-top:2px">${escHtml(entry.url)}</div>
