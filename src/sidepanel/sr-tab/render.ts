...
export function buildSrTabHtml(): string {
  ...
  return `
    <div class="fs-0" style="padding:8px 12px;border-bottom:1px solid var(--ds-zinc-200);display:flex;gap:8px;background:var(--ds-zinc-50)">
      <button type="button" id="sr-analyze" class="f-1 cur-pointer min-h-24" style="padding:8px;font-size:12px;font-weight:800;color:var(--ds-amber-cta-fg);background:var(--ds-amber-500);border:none;border-radius:4px">Analyze</button>
      ...
      ${srAnalyzed ? '<button type="button" id="sr-clear" class="cur-pointer min-h-24" style="padding:4px 10px;font-size:11px;font-weight:700;color:var(--ds-red-600);border:1px solid var(--ds-red-200);border-radius:4px;background:none">Clear</button>' : ""}
    </div>
    ${scopeSelector ? `
      <div class="fs-0" style="padding:4px 12px;background:var(--ds-blue-50);border-bottom:1px solid var(--ds-blue-100);display:flex;align-items:center;gap:6px">
        ...
        <button type="button" id="sr-clear-scope" class="cur-pointer" style="font-size:10px;font-weight:700;color:var(--ds-red-600);border:none;background:none;padding:2px 4px">Clear scope</button>
      </div>
    ` : ""}
    <div role="status" aria-live="polite" aria-atomic="true" class="fs-0" style="padding:8px 12px;border-bottom:1px solid var(--ds-zinc-200);display:flex;align-items:center;gap:8px;${playState === "playing" || playState === "paused" ? "background:var(--ds-amber-50)" : ""}">
      ...
      ${elements.length > 0 ? `
        ${playState === "idle" || playState === "complete" ? `
          <button type="button" id="sr-play-all" aria-label="Play all — read all elements aloud" class="cur-pointer" style="width:24px;height:24px;display:flex;align-items:center;justify-content:center;border:1px solid var(--ds-amber-300);border-radius:4px;background:none;color:var(--ds-amber-700)">
            <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path d="M2 1l7 4-7 4z"/></svg>
          </button>
        ` : ""}
        ${playState === "playing" ? `
          <button type="button" id="sr-pause" aria-label="Pause speech" class="cur-pointer" style="width:24px;height:24px;display:flex;align-items:center;justify-content:center;border:1px solid var(--ds-amber-300);border-radius:4px;background:none;color:var(--ds-amber-700)">
        ...
        ` : ""}
        ${playState === "paused" ? `
          <button type="button" id="sr-resume" aria-label="Resume speech" class="cur-pointer" style="width:24px;height:24px;display:flex;align-items:center;justify-content:center;border:1px solid var(--ds-amber-300);border-radius:4px;background:none;color:var(--ds-amber-700)">
            <svg width="10" height="10" viewBox="0 0 10 10" fill="currentColor" aria-hidden="true"><path d="M2 1l7 4-7 4z"/></svg>
          </button>
        ` : ""}
        ${playState === "playing" || playState === "paused" ? `
          <button type="button" id="sr-stop" aria-label="Stop speech" title="Stop speech (Esc)" class="cur-pointer" style="width:24px;height:24px;display:flex;align-items:center;justify-content:center;border:1px solid var(--ds-red-200);border-radius:4px;background:none;color:var(--ds-red-600)">
