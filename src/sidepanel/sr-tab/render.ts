...
export function buildSrTabHtml(): string {
  ...
  return `
    <div class="fs-0" style="padding:8px 12px;border-bottom:1px solid var(--ds-zinc-200);display:flex;gap:8px;background:var(--ds-zinc-50)">
      <button id="sr-analyze" class="f-1 cur-pointer min-h-24" style="padding:8px;font-size:var(--ds-text-md);font-weight:800;color:var(--ds-amber-cta-fg);background:var(--ds-amber-500);border:none;border-radius:4px">Analyze</button>
      ...
      ${srAnalyzed ? '<button id="sr-clear" class="cur-pointer min-h-24" style="padding:4px 10px;font-size:var(--ds-text-base);font-weight:700;color:var(--ds-red-600);border:1px solid var(--ds-red-200);border-radius:4px;background:none">Clear</button>' : ""}
    </div>
    ${scopeSelector ? `
      <div class="fs-0" style="padding:4px 12px;background:var(--ds-blue-50);border-bottom:1px solid var(--ds-blue-100);display:flex;align-items:center;gap:6px">
        ...
        <button id="sr-clear-scope" class="cur-pointer" style="font-size:var(--ds-text-sm);font-weight:700;color:var(--ds-red-600);border:none;background:none;padding:2px 4px">Clear scope</button>
      </div>
    ` : ""}
    ...
    <div id="sr-list" class="f-1" style="overflow-y:auto">
      ${elements.length === 0
        ? '<div style="padding:16px;text-align:center;font-size:var(--ds-text-md);color:var(--ds-zinc-500)">Click Analyze to scan the page reading order.</div>'
